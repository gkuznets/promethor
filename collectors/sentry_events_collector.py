...
class TokenAuth(requests.auth.AuthBase):
    def __init__(self, auth_token: bytes):
...
class SentryEvents(object):
    def __init__(self, config_path, timeout=None,
        ...
        self._base_api_url = config["base_api_url"]
        self._metrics = [Metric(m["name"], m["organization"], m["project"], m["horizon"]) for m in config["metrics"])
